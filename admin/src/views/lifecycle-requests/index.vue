...
import { listLifecycleRequests, reviewLifecycleRequest } from '@/api/lifecycle'
import type { LifecycleRequest } from '@/types/api'
import { formatTime } from '@/utils/format'

const loading = ref(false)
const list = ref<LifecycleRequest[]>([])
const total = ref(0)
const query = reactive({ page: 1, page_size: 10, status: undefined as number | undefined })

const dialogVisible = ref(false)
const current = ref<LifecycleRequest | null>(null)
...
async function fetchList() {
  loading.value = true
  try {
    const resp = await listLifecycleRequests({ ...query })
...
function openReview(row: LifecycleRequest) {
  current.value = row
...
async function submit() {
  ...
  try {
    await reviewLifecycleRequest(current.value.id, { ...form })
...
<template>
  <PageContainer title="注销申请">
    ...
    <el-dialog v-model="dialogVisible" title="注销审核" width="480px">
