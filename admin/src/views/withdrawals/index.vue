...
import { handleWithdrawal, listWithdrawals } from '@/api/withdrawals'
...
async function fetchList() {
  loading.value = true
  try {
    const resp = await listWithdrawals({ ...query })
    list.value = resp.withdrawals ?? []
...
function openHandle(row: Withdrawal) {
  current.value = row
...
async function submit() {
  ...
  try {
    await handleWithdrawal(current.value.id, { ...form })
...
<template>
  <PageContainer title="提现审核">
    ...
    <el-table v-loading="loading" :data="list" border stripe>
      ...
      <el-table-column prop="bankInfo" label="银行信息" show-overflow-tooltip />
    ...
    <el-dialog v-model="dialogVisible" title="提现审核" width="480px">
